...
halifax <- tibble(
  date = seq.Date(date("2021-06-01"), date("2024-04-01"), by = "month"),
  halifax_prices = c(
...
px04 <- devs %>%
  ...
  geom_ribbon(
    aes(
      ymin = ymin / 1000, ymax = ymax / 1000, fill = Developer
    ), alpha = .1
  ) +
...
- Prices continue their upward momentum since the start of 2024 with **developer list prices rising by `r current_change[,15]`** in the period of Apr-May 2024 (v/s Halifax = `r current_change[,4]`, v/s Nationwide = `r current_change[,12]`). 

- Across all indices, **prices are remain above their levels from this time last year** (∆NewBuild = `r current_change[,14]`, ∆Halifax = `r current_change[,3]`, ∆Nationwide = `r current_change[,11]`).

- Prices for the largest 4+ bedroom homes show strong annual growth across all developers while, smaller property sizes are more mixed in terms of year-on-year performance.

- The North-East and Eastern England show the strongest growth in prices across all regions in England, Wales and Scotland. 
...
-   List-prices for the largest 4+ bedroom homes have risen sharply for Barratt and Persimmon developments. Barratt also shows a sharp increase for the smallest property sizes.
...
p_reg <- d_reg %>%
  ...
  scale_y_continuous(
    labels = scales::dollar_format(prefix = "£", scale = 1),
    ) +
  ylab("") + xlab("")
