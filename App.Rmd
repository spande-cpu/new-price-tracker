...
halifax <- tibble(
  date = seq.Date(date("2021-06-01"), date("2024-03-01"), by = "month"),
  halifax_prices = c(
...
- Across all indices, **prices are still above their levels from this time last year** (∆NewBuild = `r current_change[,14]`, ∆Halifax = `r current_change[,3]`, ∆Nationwide = `r current_change[,11]`).

- However after 3-consecutive months of growth, **developer list prices have contracted by `r current_change[,15]`** in the period of Mar-Apr 2024 (v/s Halifax = `r current_change[,4]`, v/s Nationwide = `r current_change[,12]`). 
