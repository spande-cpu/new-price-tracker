...
halifax <- tibble(
  date = seq.Date(date("2021-06-01"), date("2024-09-01"), by = "month"),
  halifax_prices = c(
    260973, 261165, 263162, 267516, 270184, 272778, 275996, ## 2021
    276645, 279741, 283001, 286242, 289666, 293586, 293173, 293992, 293664,
    292406, 285425, 281713, ## 2022
    282360, 285660, 287891, 286662, 286234, 286011, 284852, 279793, 278985, 
    282221, 284039, 287244, ## 2023
    290608, 291338, 288781, 288862, 288931, 289042, 291585, 292540, 293399
  ),
  halifax_change = c(
  ...
  annual_hfx_change = c(
    ...
    -.8, 1.8, ## 2023
    2.5, 2.9, .4, 1.1, 1.6, 1.9, 2.4, 4.3, 4.7
  )
...
dev_cols <- viridis::viridis(option = "magma", n = 3)
fills <- c(
...
px04$labels$fill <- ""
px04$labels$colour <- ""
px04 <- ggplotly(px04, tooltip = c("fill","text1", "text2", "text3")) %>%
  layout(hovermode = "x")
...
- Prices **move upwards by `r current_change[,15]`** in the period of Aug-Sep 2024 (v/s Halifax = `r current_change[,4]`, v/s Nationwide = `r current_change[,12]`) after 
the first rate-cut by the BoE. 

- Across all indices, **prices hold above their levels from this time last year** (∆NewBuild = `r current_change[,14]`, ∆Halifax = `r current_change[,3]`, ∆Nationwide = `r current_change[,11]`).

- In annual terms, **Persimmon-Homes has seen the largest price-increases**.

- Prices across the regional-mix highlight some regions where prices remain 
below last-year's levels.

- **Prices for new-build homes should likely remain more resilient** [v/s the market average] **due to supply-side factors** and may see stronger positive growth as the interest-rate cycle continues to loosen.
...
pp31 <- devs %>%
  ...
  geom_ribbon(aes(ymin = ymin / 1000, 
                  ymax = ymax / 1000, 
                  fill = Developer), alpha = .1) +
  geom_line(aes(x = date(Date), y = price / 1000, col = Developer, 
                fill = Developer, group = Developer, 
                text1 = Date,
                text2 = Price,
                text3 = Credible)) +
  facet_grid(Facet ~ rooms_min.f, scales = "free_y") +
...
-   Developer pricing is showing signs of positive momentum.

-   Persimmon-Homes shows the strongest annual price growth across all property sizes relative to a mixed outlook for the other two developers.
...
-   Move to the next panel to see how prices for new-build properties are trending across UK regions.
...
d_lon <- interactions::cat_plot(
  ...
  mutate_if(is.numeric, function(x) x + fixef(model2)["european_electoral_regionLondon"]) %>%
  mutate(european_electoral_region = "London") %>%
  select(Month, rooms_min.f, european_electoral_region, price, ymin, ymax)

d_reg <- bind_rows(d_reg, d_lon) %>%
  select(Month, rooms_min.f, european_electoral_region, price, ymin, ymax) %>%
  mutate_if(is.numeric, function(x) exp(x) - 1) %>%
  group_by(european_electoral_region, rooms_min.f) %>%
  mutate(
...
p_reg1 <- d_reg %>%
  mutate(Facet = "Annual % Change") %>%
  ggplot(aes(Month, `12m%Change`)) +
  geom_line(aes(
    group = european_electoral_region,
    colour = Region,
    text1 = Date,
    text2 = `Annual Change`,
    text3 = Region
  )) +
  facet_grid(Facet ~ Rooms, scales = "free_y") +
  theme_dark() +
  theme(axis.text.x = element_text(angle = 45)) +
...
p5 <- ggplotly(p_reg1, tooltip = c("text1", "text2", "text3", "text4"))
p5 <- clean_plotly_legend(p5) 
p5$x$layout$legend$title$text <- ""

subplot(p4, p5 %>%
  layout(hovermode = "x"), nrows = 2) %>%
  clean_plotly_legend() 
```

------------------------------------------------------------------------

-   This chart compares regional trends in developer list-prices across regions, double-click on a legend item to isolate a trace corresponding to a specific region. 

-   London remains the most expensive region in the UK by-far, commanding a nearly `r scales::percent(exp(as.numeric(fixef(model2)["european_electoral_regionLondon"])) - 1)` premium on a comparable development within the East-Midlands.

-   Relative to the East-Midlands, the South-East, Eastern, South-West and West-Midlands tend to command premiums of approx. `r paste0(round((exp(as.numeric(fixef(model2)["european_electoral_regionSouth East"]))-1)*100,1),"%")`, `r paste0(round((exp(as.numeric(fixef(model2)["european_electoral_regionEastern"]))-1)*100,1),"%")`, `r paste0(round((exp(as.numeric(fixef(model2)["european_electoral_regionSouth West"]))-1)*100,1),"%")` and `r paste0(round((exp(as.numeric(fixef(model2)["european_electoral_regionWest Midlands"]))-1)*100,1),"%")` respectively.

-   Conversely, the North-East, North-West and Scotland tend to be relatively cheaper with an equivalent property in the East-Midlands commanding a premium of `r paste0(-round((exp(as.numeric(fixef(model2)["european_electoral_regionNorth East"]))-1)*100,1),"%")`, `r paste0(-round((exp(as.numeric(fixef(model2)["european_electoral_regionNorth West"]))-1)*100,1),"%")` and `r paste0(-round((exp(as.numeric(fixef(model2)["european_electoral_regionScotland"]))-1)*100,1),"%")` respectively.

-   Relative to the East-Midlands, a similar development in Wales is about `r paste0(round((exp(as.numeric(fixef(model2)["european_electoral_regionWales"]))-1)*100,1),"%")` more expensive while in Yorkshire and the Humber, about `r paste0(round((exp(as.numeric(fixef(model2)["european_electoral_regionYorkshire and The Humber"]))-1)*100,1),"%")` cheaper. Yet, prices in these regions are often comparable and not cleanly distinguishable from the East-Midlands average.
